...
| `tasks` | `tasks` | 顶层任务标签页与任务集合语义 |
| `AgentOrchestratorBoard` | `TaskBoard` | 前端任务主面板组件名 |
| `isAgentOrchestrator` | `isTaskRecord` | 前端任务判断工具函数 |
| `participated_tasks` | `participated_tasks` | Agent 总览统计字段 |
| `official` | `owner` | 任务责任字段 |
| `imperial-agentorchestrator` | `task-created` | 内部触发名 |

## Todo

- [x] 复核前端 `App.tsx`、`store.ts`、`api.ts` 与关键面板组件中的旧命名边界。
- [x] 复核后端任务模型与实时刷新脚本中 `official` / `officials` 相关主链路依赖。
- [x] 确定本轮统一命名映射执行稿，明确从 `officials/memorials/tasks` 切换到 `agents/archives/tasks`。
...
- [ ] 重构前端 API 类型 `agentorchestrator/frontend/src/api.ts`，切换 Agent 总览类型、函数名与讨论接口字段名。
- [ ] 重构前端状态仓库 `agentorchestrator/frontend/src/store.ts`，切换 `selectedOfficial`、`loadOfficials`、`isAgentOrchestrator` 与标签键。
- [ ] 重构前端主入口与组件文件，包括 `App.tsx`、`OfficialPanel.tsx`、`MemorialPanel.tsx`、`AgentOrchestratorBoard.tsx` 及相关导入。
...
| 类别 | 文件 | 风险说明 |
| --- | --- | --- |
| 后端数据模型 | `agentorchestrator/backend/app/models/task.py` | 仍通过 `official` 输出责任角色，必须与前端 `owner` 字段同步切换。 |
| 实时汇总脚本 | `scripts/refresh_live_data.py` | 仍读取 `officials_stats.json` 并输出 `officials`、`officialCount`、`history[].official`。 |
| 前端 API | `agentorchestrator/frontend/src/api.ts` | 仍定义 `OfficialInfo`、`OfficialsData` 与 `officialsStats()`。 |
| 前端状态 | `agentorchestrator/frontend/src/store.ts` | 仍使用 `OfficialsData`、`selectedOfficial`、`loadOfficials`、`isAgentOrchestrator`。 |
| 前端主入口 | `agentorchestrator/frontend/src/App.tsx` | 仍通过 `OfficialPanel`、`MemorialPanel`、`AgentOrchestratorBoard` 组织主界面。 |
