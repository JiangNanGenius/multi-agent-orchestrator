...
| 前端状态层 | 轮询拉取、缓存全局状态、分发到不同面板 | 主要由 `edict/frontend/src/store.ts` 完成 |
| 展示层 | 各面板按共享状态做视图与动作入口 | 如 `EdictBoard.tsx`、`AutomationPanel.tsx` 等 |
...
| `EdictBoard.tsx` 等组件 | 具体展示与操作入口 | 是否存在界面文案和真实行为不一致 |
