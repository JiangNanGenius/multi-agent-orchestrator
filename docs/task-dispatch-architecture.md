# 多Agent智作中枢任务分发流转体系 · 业务与技术架构

> 本文档详细阐述 **多Agent智作中枢** 如何从**业务治理设计**到**代码实现细节**，完整处理复杂多Agent协作中的任务分发、审核、调度与交付。这是一个**制度化的 AI 多 Agent 协作框架**，而非传统的自由讨论式系统；文中如出现旧状态代号，仅用于兼容历史实现。

**文档概览图**

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
业务层：协作治理模型 (Collaborative Governance Model)
  ├─ 分权制衡：用户 → 总控中心 → 规划中心 → 评审中心 → 调度中心 → 专业执行组
  ├─ 制度约束：不可越级、状态严格递进、评审必审核
  └─ 质量保障：可退回修订、实时可观测、紧急可干预
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
技术层：OpenClaw 多Agent 编排 (Multi-Agent Orchestration)
  ├─ 状态机：9个运行状态（兼容历史内部代号 Pending → Taizi → Zhongshu → Menxia → Assigned → Doing/Next → Review → Done/Cancelled）
...
**多Agent智作中枢**采用**“制度化协作”模式**，以现代中文角色体系表达分层治理：
...
```
      ...
      ├─ 接到审核通过方案
      ├─ 分析派发给哪个专业角色
      ├─ 调用专业执行组（文案/数据/代码/合规/部署/Agent管理）执行
      ├─ 监控进度 → 汇总结果
      └─ 权限：只能调用 专业执行组（不能越权调规划中心）
                │
                ├─ 文案专家 (Libu)           - 文档编制与总结
                ├─ 数据专家 (Hubu)           - 数据分析与量化
                ├─ 代码专家 (Bingbu)         - 工程实现与改造
                ├─ 合规专家 (Xingbu)         - 测试审查与安全把关
                ├─ 部署专家 (Gongbu)         - 基础设施与交付部署
                └─ Agent管理专家 (Libu_hr)   - Agent编排与治理支持
                │
          (各专家串行或并行执行)
                ↓
          调度中心·汇总
...
| 保障机制 | 实现细节 | 防护效果 |
|---------|---------|---------|
| **制度性审核** | 评审中心必审核所有规划中心方案，不可跳过 | 防止 Agent 胡乱执行，确保方案具有可行性 |
| **分权制衡** | 权限矩阵：谁能调谁严格定义 | 防止权力滥用（如调度中心越权要求规划中心重写方案） |
...
stateDiagram-v2
[*] --> Pending: 用户提交任务
Pending --> Taizi: 总控中心接收（兼容内部代号）
Taizi --> Zhongshu: 总控中心转交规划中心（兼容内部代号）
Zhongshu --> Menxia: 规划中心提交评审中心审核（兼容内部代号）
Menxia --> Zhongshu: 评审中心退回规划中心修订（可多次，兼容内部代号）
Menxia --> Assigned: 评审中心审核通过（兼容内部代号）
Assigned --> Doing: 调度中心派发至专业执行组（兼容内部代号）
Doing --> Review: 专业执行组完成并进入交付复核（兼容内部代号）
Review --> Done: 用户确认交付完成（兼容内部代号）
Review --> Menxia: 用户要求修改并回到评审链路（兼容内部代号）
Done --> [*]
Doing --> [*]: 手动取消
Review --> [*]: 业务终止
```
...
DAY 4-5:
  ...
  - 代码专家："✅ 已实现 16 个单元测试"
  - 合规专家："🔄 正在编写集成测试（8/12 完成）"
  - 文案专家："等待代码专家完成再写报告"

DAY 5:
  14:00 - 专家执行组完成
          state: Doing → Review（内部状态代号）, org: 代码专家 → 调度中心
          调度中心汇总："所有测试已完成，通过率 98.5%"
          转回规划中心
          
  15:00 - 规划中心整理交付并回复用户
          state: Review → Done（内部状态代号）
          模板回复消息，含最终成果链接和总结
```
...
DAY 3-4：
  ...
  18:00 - 评审中心重新审核
          判定："✅ 本次通过"
          flow_log 记录："评审中心 → 调度中心：✅ 审核通过（第2轮）"
          state: Menxia → Assigned → Doing（内部状态代号）
          后续同理想路径...
...
```json
{
  ...
  "title": "为多Agent智作中枢编写完整自动化测试方案",
  "official": "规划负责人",            // 当前负责角色
  "org": "规划中心",                   // 当前负责中心 / 专家组
  "state": "Assigned",               // 当前状态（见 _STATE_FLOW；内部代号兼容历史实现）
  
  // ──── 质量与约束 ────
  "priority": "normal",              // 优先级：critical/high/normal/low
  "block": "无",                     // 当前阻滞原因（如"等待部署专家反馈"）
  "reviewRound": 2,                  // 评审中心审核第几轮
  ...
  "flow_log": [
    {
      ...
      "from": "用户",
      "to": "总控中心",
      "remark": "提交任务：为多Agent智作中枢编写完整自动化测试方案"
    },
    {
  ...
  "_scheduler": {
    ...
    "maxRetry": 1,                    // 自动重试最多1次
    "retryCount": 0,
    "escalationLevel": 0,             // 0=无升级 1=评审中心协调 2=调度中心协调
  ...
  // ──── 生命周期 ────
  "archived": false,                 // 是否归档
  "now": "评审中心审核通过，已移交调度中心派发",  // 当前实时状态描述（对外显示采用现代中文标签）
...
| 契约 | 含义 | 违反后果 |
|------|------|---------|
| **不可越级** | 总控中心只能调规划中心，规划中心只能调评审中心/调度中心，专业执行组不能对外调用 | 超权调用被拒绝，系统自动拦截 |
| **状态单向递进** | Pending → Taizi → Zhongshu → ... → Done（内部状态代号链路），不能跳过或倒退 | 只能通过 `review_action(reject)` 返回上一步 |
| **评审必经** | 所有规划中心提出的方案都要进入评审中心审核，无法跳过 | 规划中心不能直接转调度中心执行，评审中心必须参与 |
| **完成后不可直接改写** | 任务进入 Done / Cancelled 后不能再修改状态 | 若需修改，需要创建新任务或取消后重新建立 |
...
```python
_STATE_FLOW = {
    'Pending':  ('Taizi',   '用户',       '总控中心',   '待处理任务转交总控中心预处理'),
    'Taizi':    ('Zhongshu','总控中心',   '规划中心',   '总控中心预处理完毕，转规划中心起草'),
    'Zhongshu': ('Menxia',  '规划中心',   '评审中心',   '规划中心方案提交评审中心审核'),
    'Menxia':   ('Assigned','评审中心',   '调度中心',   '评审中心审核通过，转调度中心派发'),
    'Assigned': ('Doing',   '调度中心',   '专业执行组', '调度中心开始派发执行'),
    'Next':     ('Doing',   '调度中心',   '专业执行组', '待执行任务开始执行'),
    'Doing':    ('Review',  '专业执行组', '调度中心',   '专家处理完成，进入汇总'),
    'Review':   ('Done',    '调度中心',   '规划中心',   '全流程完成，转规划中心整理并回复用户'),
}
```
...
```
1. 状态转移触发派发
   ├─ 查表 `_STATE_AGENT_MAP` 得到目标 Agent ID
   ├─ 若是 Doing/Next，从 `task.org` 查表 `_ORG_AGENT_MAP` 推断具体专家 Agent
...
| 场景 | 请求 | 结果 | 理由 |
|------|------|------|------|
| **正常** | 规划中心 → 评审中心审核 | ✅ 允许 | 评审中心在规划中心的 `allowAgents` 中 |
| **违反** | 规划中心 → 调度中心改方案 | ❌ 拒绝 | 规划中心只能按既定链路发起，不应直接改写调度中心执行内容 |
| **违反** | 部署专家 → 调度中心“我完成了” | ✅ 改状态 | 通过 `flow_log` 和 `progress_log` 回写，不属于跨 Agent 调用 |
| **违反** | 调度中心 → 规划中心“重新改方案” | ❌ 拒绝 | 调度中心不应绕过评审链路直接回写规划方案 |
| **防控** | Agent 伪造其他 Agent 派发 | ❌ 拦截 | API 层验证 HTTP 请求来源 / 签名 |
...
```
1️⃣ `flow_log`
   └─ 纯粹记录状态转移（如规划中心 → 评审中心；内部仍兼容 `Zhongshu → Menxia` 代号）
   └─ 数据源：任务 JSON 的 flow_log 字段
   └─ 来自：Agent 通过 kanban_update.py flow 命令上报

2️⃣ `progress_log`
   └─ Agent 的实时工作汇报（文本进展、待办快照、资源消耗）
...
3️⃣ Session JSONL（新增）
   ...
   └─ 来自：OpenClaw 框架自动记录，Agent 无需主动操作
   └─ 周期：消息级别，粒度最细
```
...
kind    count  代表事件
────────────────────────────────────────────────
flow      10   状态转移链（如总控中心→规划中心；内部兼容 `Pending→Taizi→Zhongshu→...` 代号）
progress  11   Agent 工作汇报（“正在分析”“已完成”等）
...
看板展示时，用户可以：
- 📋 看流转链了解任务在哪个阶段
- 📝 看 `progress` 了解 Agent 实时汇报了什么
- ✅ 看 todos 了解任务拆解和完成进度
- 💭 看 `assistant / thinking` 了解 Agent 的思考过程
- 🔧 看 tool_result 了解每次工具调用的结果
- 👤 看 user 了解是否有人工干预

---

### 2.4 调度系统：超时重试、停滞升级与自动回滚
...
FOR EACH 任务:
  ...
  IF escalationLevel >= 2 AND autoRollback:
    ...
    - retryCount = 0
    - escalationLevel = 0
    - `dispatch_for_state(task, snapshot.state, trigger='taiji-auto-rollback')`  # 触发器字符串保持兼容
    - flow_log: "连续停滞，自动回滚到{snapshot.state}"
```

#### 示例场景

**场景：规划中心 Agent 进程崩溃，任务卡在 `Zhongshu` 内部状态**
...
T+720 (若仍未解决):
  ...
  结果：
  - 调度中心重新派发给专业执行组执行
  - 规划中心的方案保留在前一个 `snapshot` 版本中
  - 用户可以看到回滚操作，决定是否介入
```

---

## 第三部分：核心 API 与 CLI 工具
...
响应：
{
  ...
  "message": "任务 JJC-20260228-001 已创建，正在流转至总控中心"
}
```
...
响应：
{
  ...
  "activity": [
    ...
    // - assistant (10条)
    {
      ...
      "thinking": "💭 考虑到项目的复杂性，需要覆盖多个中心与专业 Agent 的协作逻辑。单元测试应采用 pytest，集成测试可基于 `server.py` 启动后的 HTTP 流程验证...",
      "tools": [
...
### CrewAI / AutoGen 的传统方式 vs 多Agent智作中枢的制度化方式

| 维度 | CrewAI | AutoGen | **多Agent智作中枢** |
|------|--------|---------|----------|
| **协作模式** | 自由讨论（Agent自主选择协作对象） | 面板+回调（Human-in-the-loop） | **制度化协作（权限矩阵+状态机）** |
| **质量保障** | 依赖Agent智能（无审核）| Human审核（频繁中断） | **自动审核（评审中心必审）+可干预** |
...
| **吞吐量** | 1任务1Agent（串行讨论） | 1任务1Team（需人工管理） | **多任务并行（专业执行组同时执行）** |
...
# 总控中心 Agent 收到通知：“📜 新任务已进入总控中心...”

# ═══════════════════════════════════════════════════════════
# 第2步：总控中心预处理分拣（Agent 自动）
# ═══════════════════════════════════════════════════════════

# 总控中心 Agent 判定：这是正式任务（非闲聊）
# 自动运行：
python3 scripts/kanban_update.py state \
  JJC-20260302-001 \
  Zhongshu \
...
python3 scripts/kanban_update.py flow \
  ...
  "评审中心" \
  "规划中心" \
  "🚫 退回修订：协议部分过于简略，需补充权限矩阵示例"

# 规划中心 Agent 收到唤醒，重新修改方案
# （3小时后 → 重新提交评审中心审议）
...
python3 scripts/kanban_update.py state \
  JJC-20260302-001 \
  Doing \
  "派发给文案专家 + 代码专家 + 部署专家并行执行"

python3 scripts/kanban_update.py flow \
  JJC-20260302-001 \
  "调度中心" \
  "专业执行组" \
  "派发执行：文案专家排版|代码专家示例|部署专家基础设施部分"
...
python3 scripts/kanban_update.py done \
  ...
  "多中心协作协议文档已完成，包含 89 页，5 个阶段历时 3 天，总消耗成本 $2.34"
...
**多Agent智作中枢是一个制度化的 AI 多 Agent 系统**，不是传统的“自由讨论”框架。它通过：

1. **业务层**：建立分层制衡的协作结构，明确入口治理、规划、评审、调度与执行的边界
2. **技术层**：状态机 + 权限矩阵 + 自动派发 + 调度重试，确保流程可控
3. **观测层**：运行看板 + 完整活动流，实时掌握全局，并支持上下文窗口管理提示
4. **介入层**：支持 stop / cancel / advance 等操作，遇到异常能立即纠正

**核心价值**：用制度确保质量，用透明确保信心，用自动化确保效率。

相比 CrewAI / AutoGen 的“自由 + 人工管理”，多Agent智作中枢提供了一套**企业级 AI 协作框架**。
...
This is the stage shown in the board/UI and reflects where the task is in the intended process. In the current product surface, these internal states are mapped to modern Chinese labels such as 总控中心、规划中心、评审中心、调度中心 and 专业执行组 for user-facing display.
