...
| 前端操作层 | `agentorchestrator/frontend/src/*` | 任务发布、详情查看、归档回迁、搜索与技能入口 | 补齐任务工作区与归档操作入口 [2] |
...
如果工作区、账本、归档、回迁、看门狗和 `/new` 规则只存在于后端结构中，而正式前端看不到、点不到、也无法理解，那么这些机制对实际使用者而言几乎等于不存在。因此，本轮除了底座改造之外，还同步补齐了前端字段与操作入口。[2] [5]
...
综上，当前项目的真实技术形态可以概括为：**以任务模型为事实源，以状态机与事件驱动为推进骨架，以任务工作区和文件化账本为稳定交接面，以冷热分层、归档回迁和看门狗为治理增强层，以正式前端详情与操作界面为公开操作面**。[1] [2] [3] [4] [5]
...
[2]: ./technical-architecture.md "任务工作区与账本改造 Todo"
