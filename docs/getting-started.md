...
> 默认开发地址：`http://127.0.0.1:5173`，并代理到 `http://127.0.0.1:8000`。

> 如需继续查看旧版看板，请使用兼容模式：`AGENTORCHESTRATOR_ENABLE_LEGACY_DASHBOARD=1 ./agentorchestrator.sh start`，再访问 `http://127.0.0.1:7891`。
...
如果你正在联调前端，可打开开发界面 `http://127.0.0.1:5173`；若启用了兼容看板，也可访问 `http://127.0.0.1:7891`。
