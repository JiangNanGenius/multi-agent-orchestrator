...
**📜 结果归档**：已完成任务可在看板中显式归档，展示完整的五阶段时间线——总控中心预处理→规划中心→评审中心→专业执行组→结果回传；归档后还需经过最终交付检查，再正式返回结果。一键复制为 Markdown。

![结果归档](screenshots/08-archives.png)
...
还有 Agent 总览（Token 消耗排行榜）、技能管理、全网搜索简报（公开信息聚合）、会话监控，以及每天首次打开时的开场动画。运行中的长任务如果接近上下文上限，还会触发“上下文窗口管理”机制，提供预警、压缩归档与续写提示。

**运行入口保留零依赖链路**：手写看板仍然是纯 HTML + CSS + JavaScript，可以独立运行；同时项目也提供了 React 前端，便于后续扩展。

![Agent 总览](screenshots/06-agents-overview.png)
*▲ Agent 总览：Token 消耗排行榜 + 活跃度统计*

![全网搜索简报](screenshots/10-web-search-briefing.png)
*▲ 全网搜索简报：按主题聚合公开信息并生成订阅摘要*
