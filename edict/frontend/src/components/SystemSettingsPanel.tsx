    ...
    <div style={{ display: 'grid', gap: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(320px, 0.95fr) minmax(360px, 1.05fr)', gap: 16, alignItems: 'start' }}>
        <div style={{ padding: 18, borderRadius: 18, border: '1px solid var(--line)', background: 'var(--panel2)', display: 'grid', gap: 16 }}>
          <div style={{ display: 'grid', gap: 6 }}>
            <div style={{ fontSize: 18, fontWeight: 800 }}>{pickLocaleText(locale, '系统偏好', 'System Preferences')}</div>
            <div style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.7 }}>
              {pickLocaleText(locale, '这里集中管理系统级偏好，包括界面语言和巡检记录保留策略。语言切换会立即生效，并在刷新后保持。', 'This section centralizes system-level preferences, including interface language and scan-record retention. Language changes take effect immediately and stay active after refresh.')}
            </div>
          </div>
          ...
          <div style={{ padding: 14, borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', display: 'grid', gap: 10 }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>{pickLocaleText(locale, '界面语言', 'Interface Language')}</div>
            <div style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.7 }}>
              {pickLocaleText(locale, '切换主界面、导航、设置中心与会议控制台的主要文案。当前偏好会保存到本地，下次进入时自动恢复。', 'Switch the main workspace, navigation, settings center, and collaboration console copy. The current preference is stored locally and restored automatically the next time you open the app.')}
            </div>
          ...
          <div style={{ padding: 14, borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', display: 'grid', gap: 10 }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>{pickLocaleText(locale, '巡检记录设置', 'Scan Record Settings')}</div>
            <div style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.7 }}>
