...
export default function App() {
  ...
  return (
    <div className="wrap">
      ...
      <div className="hdr">
        <div>
          <div className="logo">多Agent智作中枢</div>
          <div className="sub-text">OpenClaw Multi-Agent Intelligent Collaboration Hub</div>
        </div>
        <div className="hdr-r">
          <span className={`chip ${syncOk ? 'ok' : syncOk === false ? 'err' : ''}`}>
            {syncOk ? '✅ 同步正常' : syncOk === false ? '❌ 服务器未启动' : '⏳ 连接中…'}
          </span>
          <span className="chip">{activeEdicts.length} 个任务</span>
