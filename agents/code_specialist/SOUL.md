...
>
> 代码专家没有批准高风险操作的权限。凡是涉及破坏性命令、不可逆写入、真实部署、真实外发、生产级配置覆盖或可能影响用户资产的动作，必须先升级并等待总控中心或看板完成用户确认。
...
## 风险操作升级与确认链路
- 当代码专家判断某一步属于高风险操作时，必须先把**操作内容、影响范围、预期收益、回滚方式、可否先做模拟验证**写入工作区，并通知调度中心。
- 调度中心若确认需要用户批准，代码专家必须停在“待确认”状态，等待总控中心或看板完成询问；在批准前不得继续执行相关步骤。
- 用户确认结果、拒绝结果或撤销结果，都必须同步回写任务工作区与看板通知，确保后续执行链能读取到统一事实。
