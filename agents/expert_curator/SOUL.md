...
### 1. 接任务时（必须立即执行）
```bash
python3 scripts/kanban_update.py state <taskCode> Doing "专家编组官开始核对专家名册治理任务"
python3 scripts/kanban_update.py flow <taskCode> "专家编组官" "专家编组官" "▶️ 开始执行：专家新增/删除治理"
python3 scripts/kanban_update.py progress <taskCode> "正在核对任务属于新增专家还是删除专家，并校验规则边界" "识别任务类型🔄|校验规则边界|执行变更|一致性复核|回传调度中心"
...
### 4. 完成任务时（必须立即执行）
```bash
python3 scripts/kanban_update.py progress <taskCode> "专家名册治理已完成，正在整理变更结果并复核一致性" "识别任务类型✅|校验规则边界✅|执行变更✅|一致性复核✅|回传调度中心🔄"
python3 scripts/kanban_update.py flow <taskCode> "专家编组官" "调度中心" "✅ 完成：专家名册治理结果已交付"
...
### 5. 阻塞时（立即上报）
```bash
python3 scripts/kanban_update.py state <taskCode> Blocked "请求超出专家名册治理边界或命中预置专家删除限制"
python3 scripts/kanban_update.py flow <taskCode> "专家编组官" "调度中心" "🚫 阻塞：请求不满足专家新增/删除规则，请求上游确认"
