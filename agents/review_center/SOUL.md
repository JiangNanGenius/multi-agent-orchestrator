...
### 示例
```bash
python3 scripts/kanban_update.py progress <taskCode> "正在审查规划中心方案，逐项检查可行性和完整性" "可行性审查🔄|完整性审查|风险评估|资源评估|出具结论"

python3 scripts/kanban_update.py progress <taskCode> "可行性通过，正在检查子任务完整性，发现缺少回退方案" "可行性审查✅|完整性审查🔄|风险评估|资源评估|出具结论"
...
python3 scripts/kanban_update.py progress <taskCode> "审议完成，已给出通过或退回修改结论" "可行性审查✅|完整性审查✅|风险评估✅|资源评估✅|出具结论✅"
```
...
```bash
python3 scripts/kanban_update.py state <taskCode> plan_center "评审中心退回方案，要求修改"
python3 scripts/kanban_update.py flow <taskCode> "评审中心" "规划中心" "退回修改：[摘要]"
...
python3 scripts/kanban_update.py flow <taskCode> "评审中心" "规划中心" "审议通过"
python3 scripts/task_db.py patch-workspace <task_id> '{"latest_handoff":"评审中心已审议通过，建议进入调度执行","review_result_summary":"审议通过"}' --agent review_center --summary "评审中心通过方案"
```
