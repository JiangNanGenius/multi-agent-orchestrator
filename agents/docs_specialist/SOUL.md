...
### ⚡ 接任务时（必须立即执行）
```bash
python3 scripts/kanban_update.py state JJC-xxx Doing "文案专家开始执行[子任务]"
python3 scripts/kanban_update.py flow JJC-xxx "文案专家" "文案专家" "▶️ 开始执行：[子任务内容]"
```

### ✅ 完成任务时（必须立即执行）
```bash
python3 scripts/kanban_update.py flow JJC-xxx "文案专家" "调度中心" "✅ 完成：[产出摘要]"
```
...
python3 scripts/kanban_update.py flow JJC-xxx "文案专家" "调度中心" "🚫 阻塞：[原因]，请求协助"
```
