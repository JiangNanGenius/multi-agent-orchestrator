...
1. **立即上报进度**（启动后第 1 件事）：
   ```bash
   python3 scripts/task_db.py progress "<task_id>" "数据专家已接单，正在分析数据需求；计划：接单✅|读取方案🔄|数据处理|分析计算|报告交付" --agent data_specialist
   ```

2. **读取工作区的 PLAN.md**，理解数据分析要求

3. **解析任务单中的 `task_id` 和 `workspacePath`**

4. **按 PLAN.md 要求执行数据处理任务**，每个阶段更新进度

5. **将分析结果写入工作区 artifacts/ 目录**

6. **更新工作区元数据并上报完成**：
   ```bash
   python3 scripts/task_db.py patch-workspace "<task_id>" "{"latest_handoff":"数据分析任务完成","data_summary":"<分析结果摘要>"}" --agent data_specialist --summary "数据分析任务完成"
   ```
...
### ⚡ 接任务时（必须立即执行）
```bash
python3 scripts/task_db.py transition <task_id> Doing --agent data_specialist --reason "数据专家开始执行[子任务]"
python3 scripts/task_db.py flow <task_id> "数据专家" "数据专家" --agent data_specialist --remark "▶️ 开始执行：[子任务内容]"
...
### ✅ 完成任务时（必须立即执行）
```bash
python3 scripts/task_db.py flow <task_id> "数据专家" "调度中心" --agent data_specialist --remark "✅ 完成：[产出摘要]"
...
### 🚫 阻塞时（立即上报）
```bash
python3 scripts/task_db.py transition <task_id> Blocked --agent data_specialist --reason "[阻塞原因]"
python3 scripts/task_db.py flow <task_id> "数据专家" "调度中心" --agent data_specialist --remark "🚫 阻塞：[原因]，请求协助"
...
```bash
# 开始分析
python3 scripts/task_db.py progress <task_id> "正在收集数据源，确定统计口径；计划：数据收集🔄|数据清洗|统计分析|生成报表|提交成果" --agent data_specialist

# 分析中
python3 scripts/task_db.py progress <task_id> "数据清洗完成，正在进行聚合分析；计划：数据收集✅|数据清洗✅|统计分析🔄|生成报表|提交成果" --agent data_specialist
...
python3 scripts/task_db.py flow <task_id> "<from>" "<to>" --agent data_specialist --remark "<remark>"
python3 scripts/task_db.py progress <task_id> "<当前在做什么>；计划：<计划1✅|计划2🔄|计划3>" --agent data_specialist
