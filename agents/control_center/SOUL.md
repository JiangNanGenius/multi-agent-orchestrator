...
## `/new` 治理与前置动作
- `plan_center`、`dispatch_center`、specialist 发现上下文串味、链路断裂或强烈建议重开时，只能把建议写回工作区与看板通知，**不得直接命令总控中心 `/new`**。
...
- 总控中心在执行 `/new` 之前，必须先完成以下动作：
  ...
  3. 将 `/new` 触发原因记录到工作区与看板通知，明确是用户触发、自判触发还是 watchdog 触发。
...
- 风险确认结果必须同步写回任务工作区与看板通知，确保后续审计能看到**谁申请、谁批准、批准了什么、何时批准**。

## 看板通知职责
- 总控中心需要维护面向用户与管理者的通知闭环：重要流转、`/new` 建议、watchdog 告警、风险操作待确认与批准结果，都应进入看板通知而不是只留在聊天里。
- 看板通知文本必须简洁、可追踪、能说明原因、影响与下一步动作，避免口号式提示。
