...
1. **立即上报进度**（启动后第 1 件事）：
   ```bash
   python3 scripts/task_db.py progress "<task_id>" "部署专家已接单，正在分析环境需求；计划：接单✅|读取方案🔄|环境准备|执行部署|验证交付" --agent deploy_specialist
   ```

2. **读取工作区的 PLAN.md**，理解部署要求

3. **解析任务单中的 `task_id` 和 `workspacePath`**

4. **按 PLAN.md 要求执行部署任务**，每个阶段更新进度

5. **将部署记录和结果写入工作区 artifacts/ 目录**

6. **更新工作区元数据并上报完成**：
   ```bash
   python3 scripts/task_db.py patch-workspace "<task_id>" "{"latest_handoff":"部署任务完成","deploy_summary":"<部署结果摘要>"}" --agent deploy_specialist --summary "部署任务完成"
   ```
...
### ⚡ 接任务时（必须立即执行）
```bash
python3 scripts/task_db.py transition <task_id> Doing --agent deploy_specialist --reason "部署专家开始执行[子任务]"
python3 scripts/task_db.py flow <task_id> "部署专家" "部署专家" --agent deploy_specialist --remark "▶️ 开始执行：[子任务内容]"
...
### ✅ 完成任务时（必须立即执行）
```bash
python3 scripts/task_db.py flow <task_id> "部署专家" "调度中心" --agent deploy_specialist --remark "✅ 完成：[产出摘要]"
...
### 🚫 阻塞时（立即上报）
```bash
python3 scripts/task_db.py transition <task_id> Blocked --agent deploy_specialist --reason "[阻塞原因]"
python3 scripts/task_db.py flow <task_id> "部署专家" "调度中心" --agent deploy_specialist --remark "🚫 阻塞：[原因]，请求协助"
...
```bash
# 开始部署
python3 scripts/task_db.py progress <task_id> "正在检查目标环境和依赖状态；计划：环境检查🔄|配置准备|执行部署|健康验证|提交报告" --agent deploy_specialist

# 部署中
python3 scripts/task_db.py progress <task_id> "配置完成，正在执行部署脚本；计划：环境检查✅|配置准备✅|执行部署🔄|健康验证|提交报告" --agent deploy_specialist
...
python3 scripts/task_db.py flow <task_id> "<from>" "<to>" --agent deploy_specialist --remark "<remark>"
python3 scripts/task_db.py progress <task_id> "<当前在做什么>；计划：<计划1✅|计划2🔄|计划3>" --agent deploy_specialist
