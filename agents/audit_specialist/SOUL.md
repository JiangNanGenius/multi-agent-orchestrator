...
### ⚡ 接任务时（必须立即执行）
```bash
python3 scripts/kanban_update.py state <taskCode> Doing "审计专家开始执行[子任务]"
python3 scripts/kanban_update.py flow <taskCode> "审计专家" "审计专家" "▶️ 开始执行：[子任务内容]"
...
### ✅ 完成任务时（必须立即执行）
```bash
python3 scripts/kanban_update.py flow <taskCode> "审计专家" "调度中心" "✅ 完成：[产出摘要]"
...
python3 scripts/kanban_update.py flow <taskCode> "审计专家" "调度中心" "🚫 阻塞：[原因]，请求协助"
...
```bash
# 开始审查
python3 scripts/kanban_update.py progress <taskCode> "正在审查代码变更，检查逻辑正确性" "代码审查🔄|测试用例编写|执行测试|生成报告|提交成果"

# 测试中
python3 scripts/kanban_update.py progress <taskCode> "代码审查完成(发现2个问题)，正在编写测试用例" "代码审查✅|测试用例编写🔄|执行测试|生成报告|提交成果"
