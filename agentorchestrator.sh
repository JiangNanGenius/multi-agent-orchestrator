...
# 可通过环境变量覆盖的配置
DASHBOARD_HOST="${AGENTORCHESTRATOR_DASHBOARD_HOST:-127.0.0.1}"
DASHBOARD_PORT="${AGENTORCHESTRATOR_DASHBOARD_PORT:-7891}"
...
case "${1:-}" in
  ...
  *)
    ...
    echo ""
    echo "环境变量:"
    echo "  AGENTORCHESTRATOR_DASHBOARD_HOST  监听地址 (兼容键，默认: 127.0.0.1)"
    echo "  AGENTORCHESTRATOR_DASHBOARD_PORT  监听端口 (兼容键，默认: 7891)"
    exit 1
    ;;
esac
