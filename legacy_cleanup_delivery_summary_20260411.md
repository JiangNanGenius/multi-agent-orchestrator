...
本轮继续按“除首页外不保留旧体系历史材料”的标准做了进一步清理。

已完成事项包括：

- 删除首页之外承载旧体系命名或残留说明的历史扫描、审计与迁移产物。
- 删除若干归档说明与阶段性审计文件，避免仓库继续保留无效历史表述。
- 修正 `README_JA.md` 中首页外仍保留的旧来源说明，将表述收敛为中性致谢与许可说明。
- 复核 Git 跟踪文件后，首页之外已不再存在目标旧体系词的有效文本命中；仅剩 `CONTRIBUTING.md` 中一个正常英文单词的连续字母片段误命中，不属于残留。

本轮工作完成后，仓库状态已满足“首页作为例外，其余位置连历史材料也不保留”的清理目标，可进入最终提交与推送。
