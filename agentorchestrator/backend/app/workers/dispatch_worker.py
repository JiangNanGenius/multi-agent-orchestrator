...
def _resolve_agents_dir() -> pathlib.Path:
    ...
    # 默认: 相对于 agentorchestrator/backend 上溯到项目根
...
class DispatchWorker:
    ...
    async def _call_openclaw(
        ...
        env = os.environ.copy()
        env["AGENTORCHESTRATOR_TASK_ID"] = task_id
        env["AGENTORCHESTRATOR_TRACE_ID"] = trace_id
        env["AGENTORCHESTRATOR_API_URL"] = f"http://localhost:{settings.port}"

        # 注入额外上下文环境变量
        if payload:
            env["AGENTORCHESTRATOR_TASK_TITLE"] = payload.get("title", "")
            env["AGENTORCHESTRATOR_TASK_STATE"] = payload.get("state", "")
            env["AGENTORCHESTRATOR_TASK_ORG"] = payload.get("org", "")
            env["AGENTORCHESTRATOR_TASK_PRIORITY"] = payload.get("priority", "中")
            tags = payload.get("tags", [])
            if tags:
                env["AGENTORCHESTRATOR_TASK_TAGS"] = ",".join(str(t) for t in tags)
        ...
        if payload:
            ...
            try:
                fd, context_file = tempfile.mkstemp(suffix=".json", prefix=f"agentorchestrator_ctx_{task_id}_")
                with os.fdopen(fd, "w") as f:
                    json.dump(context_data, f, ensure_ascii=False, indent=2)
                env["AGENTORCHESTRATOR_CONTEXT_FILE"] = context_file
            except Exception as e:
