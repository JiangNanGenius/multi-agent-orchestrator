...
import { useStore, getPipeStatus, stateLabel, deptColor, isArchived, isAgentOrchestrator, getSchedulerSummary, DEPTS } from '../store';
...
const PROGRESS_CHECK_HISTORY_KEY = 'agentorchestrator-progress-check-history';
...
function TaskCard({ task, busyEntries }: { task: Task; busyEntries: CollabAgentBusyEntry[] }) {
  const locale = useStore((s) => s.locale);
  ...
  return (
    <div
      className={`agentorchestrator-card${archived ? ' archived' : ''}`}
...
export default function AgentOrchestratorBoard() {
  const locale = useStore((s) => s.locale);
  ...
  const allTasks = tasks.filter((task) => isAgentOrchestrator(task) && !isReleaseDemoNoiseTask(task));
  const activeTasks = allTasks.filter((t) => !isArchived(t));
  const archivedTasks = allTasks.filter((t) => isArchived(t));

  let visibleTasks: Task[];
  if (taskFilter === 'active') visibleTasks = activeTasks;
  else if (taskFilter === 'archived') visibleTasks = archivedTasks;
  else visibleTasks = allTasks;
  ...
  const spotlightTasks = activeTasks.slice(0, 4);
  const runningTasks = activeTasks.filter((t) => ['Doing', 'Assigned', 'PlanCenter', 'ControlCenter'].includes(t.state)).slice(0, 4);
  const recentDoneTasks = allTasks.filter((t) => ['Done', 'Cancelled'].includes(t.state)).slice(0, 4);
  const integratedHistoryTasks = (taskFilter === 'archived' ? archivedTasks : archivedTasks.slice(0, 6)).sort((a, b) => {
    const aTime = new Date(a.updatedAt || 0).getTime();
    const bTime = new Date(b.updatedAt || 0).getTime();
    return bTime - aTime;
  });
  const singleTaskMode = allTasks.length === 1;
  ...
  const heroMetrics = [
    ...
    },
    {
      ...
      detail: pickLocaleText(locale, `累计 ${completedCount}/${allTasks.length || 0} 已结束`, `${completedCount}/${allTasks.length || 0} completed overall`),
    },
  ];
      ...
      <section className={`mission-board__catalog-shell${visibleTasks.length === 1 ? ' mission-board__catalog-shell--single' : ''}`}>
        <div className="mission-board__section-topline">
          ...
          <div className="mission-board__hint-text">
            {locale === 'en' ? `Active ${activeTasks.length} · Archived ${archivedTasks.length} · Total ${allTasks.length}` : `活跃 ${activeTasks.length} · 归档 ${archivedTasks.length} · 共 ${allTasks.length}`}
          </div>
        </div>

        <div className={`agentorchestrator-grid${visibleTasks.length === 1 ? ' agentorchestrator-grid--single' : ''}`}>
          {visibleTasks.length === 0 ? (
            ...
            visibleTasks.map((t) => <TaskCard key={t.id} task={t} busyEntries={taskBusyMap[t.id] || []} />)
          )}
        </div>
      </section>
