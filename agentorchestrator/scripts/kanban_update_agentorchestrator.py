...
# AgentOrchestrator API 地址 — 环境变量 > 默认 localhost:8000
AGENTORCHESTRATOR_API_URL = os.environ.get('AGENTORCHESTRATOR_API_URL', 'http://localhost:8000')

# 是否启用 API 模式（AGENTORCHESTRATOR_MODE=api | json | auto）
AGENTORCHESTRATOR_MODE = os.environ.get('AGENTORCHESTRATOR_MODE', 'auto').lower()
...
# State → AgentOrchestrator TaskState value 映射
_STATE_TO_TASK = {
...
def _api_available() -> bool:
    """检查 AgentOrchestrator API 是否可用。"""
    if AGENTORCHESTRATOR_MODE == 'json':
        return False
    if AGENTORCHESTRATOR_MODE == 'api':
        return True
    # auto mode: 探测
    try:
        import urllib.request
        req = urllib.request.Request(f"{AGENTORCHESTRATOR_API_URL}/health", method='GET')
...
def _api_post(path: str, data: dict) -> dict | None:
    """向 AgentOrchestrator API 发送 POST 请求。"""
    try:
        import urllib.request
...
def _api_put(path: str, data: dict) -> dict | None:
    """向 AgentOrchestrator API 发送 PUT 请求。"""
    try:
        import urllib.request
        body = json.dumps(data, ensure_ascii=False).encode('utf-8')
        req = urllib.request.Request(
...
def _check_api():
    ...
    if _api_ok is None:
        ...
        if _api_ok:
            log.debug('AgentOrchestrator API 可用，使用 API 模式')
        else:
            log.debug('AgentOrchestrator API 不可用，降级到 JSON 模式')
    return _api_ok
...
def cmd_create(task_id, title, state, org, owner, remark=None):
    ...
    if _check_api():
        task_state = _STATE_TO_TASK.get(state, state.lower())
        ...
        if result:
            log.info(f'✅ 创建 {task_id} → AgentOrchestrator {result.get("task_id", "?")} | {title[:30]}')
            return
...
def cmd_state(task_id, new_state, now_text=None):
    if _check_api():
        task_state = _STATE_TO_TASK.get(new_state, new_state.lower())
        agent = _infer_agent_id()
        # 需要先通过 legacy_id 查找 agentorchestrator task_id
        # 暂用 legacy_id tag 搜索
