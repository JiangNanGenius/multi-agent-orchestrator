...
banner() {
  echo ""
  echo -e "${BLUE}╔══════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║  多Agent智作中枢 · OpenClaw 安装向导     ║${NC}"
  echo -e "${BLUE}║  Registry / SOUL / 工作区自动同步        ║${NC}"
  echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
  echo ""
}
...
required = [
    {"id": "taizi", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-taizi'), "subagents": {"allowAgents": ["zhongshu"]}},
    {"id": "zhongshu", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-zhongshu'), "subagents": {"allowAgents": ["menxia", "shangshu"]}},
    {"id": "menxia", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-menxia'), "subagents": {"allowAgents": ["shangshu", "zhongshu"]}},
    {"id": "shangshu", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-shangshu'), "subagents": {"allowAgents": ["zhongshu", "menxia", "hubu", "libu", "bingbu", "xingbu", "gongbu", "libu_hr"]}},
    {"id": "hubu", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-hubu'), "subagents": {"allowAgents": ["shangshu"]}},
    {"id": "libu", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-libu'), "subagents": {"allowAgents": ["shangshu"]}},
    {"id": "bingbu", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-bingbu'), "subagents": {"allowAgents": ["shangshu"]}},
    {"id": "xingbu", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-xingbu'), "subagents": {"allowAgents": ["shangshu"]}},
    {"id": "gongbu", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-gongbu'), "subagents": {"allowAgents": ["shangshu"]}},
    {"id": "libu_hr", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-libu_hr'), "subagents": {"allowAgents": ["shangshu"]}},
    {"id": "zaochao", "workspace": str(pathlib.Path.home() / '.openclaw/workspace-zaochao'), "subagents": {"allowAgents": []}},
]
...
PYEOF
  ...
  if [ -f "$REPO_DIR/data/openclaw_registry_suggestions.json" ] && grep -q 'missingAgents' "$REPO_DIR/data/openclaw_registry_suggestions.json"; then
    warn "install.sh 已切换为只读对接模式：不会直接改写 openclaw.json。"
    info "如需补齐运行时注册，请参考 data/openclaw_registry_suggestions.json 中的建议项手动处理。"
  fi
