...
ensure_backend_deps() {
  ...
  if [ ! -x "$VENV_DIR/bin/python3" ]; then
    info "创建本地 Python 虚拟环境: $VENV_DIR"
    python3 -m venv "$VENV_DIR"
  fi
  BACKEND_PYTHON="$VENV_DIR/bin/python3"

  if ! "$BACKEND_PYTHON" -m pip --version >/dev/null 2>&1; then
    error "虚拟环境中的 pip 不可用，请检查 Python venv 组件是否完整。"
    exit 1
  fi

  if ! "$BACKEND_PYTHON" - <<'PYEOF' >/dev/null 2>&1
...
PYEOF
  then
    warn "检测到官方后端依赖缺失，开始在项目虚拟环境中安装 requirements.txt ..."
    "$BACKEND_PYTHON" -m pip install --disable-pip-version-check -r "$req_file"
  fi
...
PYEOF
  then
    error "官方后端依赖仍不完整，请手动执行：$BACKEND_PYTHON -m pip install -r agentorchestrator/backend/requirements.txt"
    exit 1
  fi

  log "官方后端依赖已就绪（虚拟环境: $VENV_DIR）"
}
...
build_frontend() {
  ...
  local frontend_dir="$REPO_DIR/agentorchestrator/frontend"
  ...
  if ! command -v node &>/dev/null; then
    if [ -f "$dist_dir/index.html" ]; then
      warn "未找到 node，但仓库内已存在可用的 dist 产物，将直接复用。"
      return
    fi
    error "未找到 node，且当前没有现成的前端 dist 产物；正式部署无法完成。请先安装 Node.js 18+ 后重新运行 ./install.sh"
    exit 1
  fi
  ...
  if [ ! -f "$dist_dir/index.html" ]; then
    error "前端构建失败：未找到 dist/index.html，无法完成正式部署。"
    exit 1
  fi
...
verify_frontend_dist() {
  local dist_dir="$REPO_DIR/agentorchestrator/frontend/dist"
  if [ ! -f "$dist_dir/index.html" ]; then
    error "缺少前端部署产物 dist/index.html。当前仓库虽可启动后端，但正式界面无法访问。"
    exit 1
  fi
  log "前端部署产物校验通过"
}
...
first_sync() {
  ...
  REPO_DIR="$REPO_DIR" "$BACKEND_PYTHON" scripts/sync_agent_config.py || warn "sync_agent_config 有警告"
  "$BACKEND_PYTHON" scripts/sync_agents_overview.py || warn "sync_agents_overview 有警告"
  "$BACKEND_PYTHON" scripts/refresh_live_data.py || warn "refresh_live_data 有警告"
...
echo "     - 正式访问地址:            http://127.0.0.1:38000/"
echo "     - 健康检查:                http://127.0.0.1:38000/health"
echo "     - 说明:                    后端会直接托管已构建的正式前端，不再出现“只起后端没前端”的假完成状态"
echo "  4. 历史说明归档:              docs/project-notes/change-notes/DEV_MODE_ARCHIVED_2026-04-25.md"
echo ""
