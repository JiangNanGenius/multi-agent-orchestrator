...
| 技能配置 | 管理本地技能、远程技能与能力扩展 | 技能扩展入口 |
| 全网搜索 | 配置与查看搜索任务、抓取与搜索结果 | 检索增强入口 |

> 本轮已进一步清理 `结果归档` 与 `Agent 总览` 中残留的历史三省六部展示语义。当前公开 UI 应只显示“总控中心、规划中心、评审中心、调度中心、文案专家、数据专家、代码专家、合规专家、部署专家、管理专家、搜索专家”等现代命名；若历史数据仍包含旧值，应在映射层转换后再渲染。
