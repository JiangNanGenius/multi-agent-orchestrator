...
> **一个默认以中文部署、中文运行为主，并面向复杂任务治理场景设计的多智能体协作编排系统。**
>
> 本仓库强调**任务分阶段治理、执行过程可观测、结果可归档、角色模板可替换、看板可继续产品化扩展**。它既适合作为公开仓库展示，也适合作为部署底座、二次开发基础工程，以及供部署阶段 AI 直接读取的主入口文档。

## 项目概述

**Multi-Agent Orchestrator** 用于把复杂任务放入一条可治理、可观测、可审查、可归档的执行链路，而不是把多个 Agent 简单拼接为一个单点对话入口。系统将任务依次纳入**任务入口、总控预处理、规划拆解、评审把关、调度派发、专业执行、结果归档**等阶段，以降低多人协作与多代理协作过程中的混乱、遗漏与不可追踪风险。
...
下图用于概括公开版项目在默认治理模式下的主执行链路。它将任务从受理、治理、审查、派发到专业执行、归档与最终交付的关系压缩为一张统一流程图，便于部署阶段阅读者、二次开发者与公开仓库访客快速建立共同认知。

![Multi-Agent Orchestrator 项目流程图](docs/readme-workflow.png)

> 该流程图对应的主链路为：**任务入口 → 总控中心 → 规划中心 → 评审中心 → 调度中心 → 专业执行组 → 结果归档 → 最终交付**。其中，评审阶段允许退回规划阶段修订，调度阶段则持续监控执行进度并汇总结果。[1]
