...
def handle_task_action(task_id, action, reason):
    ...
    if action == 'stop':
        task['state'] = 'Blocked'
        task['block'] = reason or '总控中心叫停'
        task['now'] = f'⏸️ 已暂停：{reason}'
    elif action == 'cancel':
        task['state'] = 'Cancelled'
        task['block'] = reason or '总控中心取消'
    ...
    if action == 'resume':
        ...
        _scheduler_add_flow(task, f'总控中心{action}：{reason or "无"}')
...
def handle_review_action(task_id, action, comment=''):
    ...
    task.setdefault('flow_log', []).append({
        'at': now_iso(),
        'from': '评审中心' if task.get('state') != 'Done' else '总控中心',
        'to': to_dept,
        'remark': remark
    })
    ...
    label = '已通过评审' if action == 'approve' else '已退回修订'
...
def handle_scheduler_rollback(task_id, reason=''):
    ...
    task['now'] = f'↩️ 总控中心自动回滚：{reason or "恢复到上个稳定节点"}'
    task['block'] = '无'
    sched['retryCount'] = 0
    sched['escalationLevel'] = 0
    sched['stallSince'] = None
    ...
    for task in tasks:
        ...
        if sched.get('autoRollback', True):
            ...
            if rollback_count >= max_rollback:
                ...
                if state != 'Blocked':
                    task['state'] = 'Blocked'
                    task['now'] = f'🚫 连续回滚{rollback_count}次仍无法推进，已自动挂起等待人工处理'
                    task['block'] = f'连续停滞且回滚{rollback_count}次均失败，需人工介入'
                    sched['stallSince'] = None
                    _scheduler_add_flow(task, f'连续回滚{rollback_count}次，自动挂起等待人工处理')
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    _msgs = {
        ...
        'shangshu': (
            ...
            f'任务ID: {task_id}\n'
            f'任务标题: {title}\n'
            f'{"建议执行对象: " + target_dept if target_dept else ""}\n'
            f'⚠️ 看板已有此任务，请勿重复创建。\n'
            f'请分析方案并派发给对应专家执行。'
        ),
    }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == '/api/task-action':
            ...
            reason = body.get('reason', '').strip() or f'总控中心从看板{action}'
...
def main():
    parser = argparse.ArgumentParser(description='多Agent智作中枢看板服务器')
    ...
    log.info(f'多Agent智作中枢看板启动 → http://{args.host}:{args.port}')
    print(f'   按 Ctrl+C 停止')
